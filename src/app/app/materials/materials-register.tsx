...
/** One lab's materials register. Mounted at /app/materials/<slug>. */
export async function MaterialsRegister({
  slug,
  searchParams,
}: {
  slug: FacilitySlug;
  ...
  const items = await prisma.materialItem.findMany({
    ...
    where: { ...readScope(user), facilityId: facility.id, type },
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-semibold">
          Materials — {facility.route.name}
        </h1>
        {canManageMaterials(user.designation) && (
          <Link
            href={`/app/materials/${slug}/new?type=${type}`}
      ...
      <div className="flex gap-1 border-b">
        {TABS.map((t) => (
          <Link
            key={t.type}
            href={`/app/materials/${slug}?type=${t.type}`}
            className={cn(
          ...
          <TableBody>
            ...
            {items.length === 0 && (
              <TableRow>
                ...
                >
                  None registered in {facility.route.name}.
                </TableCell>
              </TableRow>
            )}
          </TableBody>
